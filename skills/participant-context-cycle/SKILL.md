...
| 4 PACE | Claude in Chrome + Andrew's live PRODA session | **Handoff checklist**: each fact to verify goes into §4 as `UNVERIFIED — verify in PACE (Route A/B)` and into §7 |
...
- Exit: every in-window comm has exactly one status and a note; Gap count stated.
...
**Deliver** the pack as one file (plan here, build there). Close with: note range covered, gap count, tasks drafted/created, open §7 items, and the reminder that it is an AI DRAFT to rewrite.
...
- OPEN QUESTION for Andrew: confirm the match tolerance (proposed ±3 days) for the gap analysis.
- OPEN QUESTION for Andrew: priority prefixes P1/P2/P3 — confirm mapping to the Urgent / Near-term / Watch pattern used by astalty-followup-task.
- OPEN QUESTION for Andrew: should the finished pack also land as a Drafting Bay row (S33), or stay a file only as in v1?
- OPEN QUESTION for Andrew: should plan-extension's Lead SC STOP-rule list (e.g. plan expiry within 30 days unconfirmed, provider red flag) also gate this cycle?
- OPEN QUESTION for Andrew: include Teams chats and Fieldy conversations as gap-analysis channels, or Outlook only?
